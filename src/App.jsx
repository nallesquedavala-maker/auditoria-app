...
const QS = {
  ...
  fund: [
    ...
    { text:"¿El retorno mensual prometido supera el 5%?",
      ...
      w:[0,-25,-40], critical:[null,"Supera el umbral de alerta CNBV. Exige modelo financiero detallado y verificable.","PATRÓN PONZI CONFIRMADO: El FBI IC3 2025 identifica este umbral como criterio #1 de fraude estructural."], src:"CNBV Circular 9/2024 / FBI IC3 Report 2025" },
    ...
    { text:"¿El promotor genera urgencia de cupo o ventana de inversión que 'cierra pronto'?",
      opts:["No — proceso de decisión sin presión temporal","Levemente — con argumentación parcial","Sí — presión constante e injustificada"],
      w:[0,-5,-15], critical:[null,null,"Urgencia artificial es el mecanismo para impedir el análisis racional del inversor."], src:"PROFECO / CONDUSEF 2025" },
  ...
  crypto: [
    ...
    { text:"¿El promotor presiona para que inviertas hoy bajo amenaza de perder el precio?",
      opts:["No — proceso de análisis respetado","Levemente — con argumentación parcial","Sí — presión constante e injustificada"],
...
function AuditorEngine() {
  ...
  const goNext = () => {
    if (curr < qs.length - 1) { setCurr(c => c + 1); setAlert(null); }
    else { setResult(calcScore(type, answers)); setPhase("result"); }
  };
  const goPrev = () => { if (curr > 0) { setCurr(c => c - 1); setAlert(null); } };
  const reset  = () => { setPhase("type"); setType(null); setName(""); setCurr(0); setAnswers({}); setResult(null); setAlert(null); };
            ...
            {phase === "name" && (
              <div style={{ animation: "fadeUp .45s ease" }}>
                ...
                <input type="text" placeholder={`Ej: ${type === "realestate" ? "Torres Platino Residencial" : type === "fund" ? "Alpha Capital Fund" : "ProInvest Token"}`}
                  ...
                  style={{ marginBottom: 20, fontSize: "14px !important" }} autoFocus />
          ...
          <div style={{ display: "flex", gap: 32, justifyContent: "center", flexWrap: "wrap", marginBottom: 48 }}>
            {[["30 días", "Para aplicar el protocolo"],["Sin formularios", "Reembolso directo"],["Sin condiciones", "Tu criterio es el juez"]].map(([t, s]) => (
              <div key={t} style={{ textAlign: "center" }}>
                <div style={{ fontSize: 20, color: T.gold, marginBottom: 5 }}>{t}</div>
